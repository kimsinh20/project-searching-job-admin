...
public class ArticleEdit extends HttpServlet {
	...
	protected void view(HttpServletRequest request, HttpServletResponse response, UserObject user)
		...
		// lấy cấu trúc
		Pair<ArticleObject, ArrayList<CategoryObject>> datas = ac.getArticle(id, user);
		ArticleObject article = datas.getValue0();
		ArrayList<CategoryObject> categories = datas.getValue1();
		...
		out.append("<h1>cập nhật bài viết & Tin tức </h1>");
		...
		out.append("<li class=\"breadcrumb-item active\">cập nhật bài viết</li>");
		out.append("</ol>");
		out.append("</nav>");
		...
		out.append("<input name=\"txtarticletitle\" value=\""+article.getArticle_title()+"\" type=\"text\" class=\"form-control\" id=\"articletitle\">");
		out.append("</div>");
		out.append("</div>");
		out.append("</div>");
		...
		out.append("<div class=\"input-group\">");
		out.append(
				"<textarea name=\"txtarticlesumary\" class=\"form-control\" id=\"articlesumary\" style=\"height: 100px\">"+article.getArticle_summary()+"</textarea>");
		out.append("</div>");
		out.append("</div>");
		out.append("</div>");
		...
		out.append("<img  width=\"400\" src=\""+article.getArticle_image()+"\" id=\"blah\" alt=\"\">");
		out.append("</div>");
		out.append("</div>");
		out.append("</div>");
		...
		categories.forEach(item -> {
			if(item.getCategory_id()==article.getArticle_category_id()) {
			out.append("<option value=\"" + item.getCategory_id() + ":" + item.getCategory_section_id() + "\" selected>");
		...
		out.append("<div class=\"col-md-9 col-lg-10\">");
		out.append(
				"<textarea name=\"txtarticlecontent\" class=\"form-control\" id=\"articlecontent\" style=\"height: 100px\">"+article.getArticle_content()+"</textarea>");
		out.append("<script>");
		...
		out.append("<div class=\"col-md-9 col-lg-10\">");
		out.append("<div class=\"input-group\">");
		out.append("<input name=\"txtarticlesource\" value=\""+article.getArticle_source()+"\" type=\"text\" class=\"form-control\" id=\"articlesource\">");
		out.append("</div>");
		out.append("</div>");
		out.append("</div>");
		...
		out.append("<div class=\"col-md-9 col-lg-10\">");
		out.append("<div class=\"input-group\">");
		out.append("<input type=\"text\" value=\""+article.getArticle_tag()+"\" id=\"todo-input\">");
...
//        System.out.println(article.getArticle_tag());
		out.append("<div class=\"text-center\">");
		out.append(
				"<button type=\"submit\" class=\"btn btn-primary\"><i class=\"far fa-edit me-2\"></i>cập nhật</button>");
		out.append("</div>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (articletitle != null && !articletitle.equalsIgnoreCase("") 
			...
			System.out.println(newar.getArticle_summary());
			System.out.println(newar.getArticle_content());
			ConnectionPool cp = (ConnectionPool) getServletContext().getAttribute("CPool");
			ArticleControl ac = new ArticleControl(cp);
			// add new user
			if (ac != null) {
...
//			boolean result = false;
			boolean result = ac.editArticle(newar,ARTICLE_EDIT_TYPE.GENERAL);
			// return connect
			ac.releaseConnection();
			if (result) {
